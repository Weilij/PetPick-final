...
// ===== 狀態 =====
// 已插入訊息的去重集合（避免同一則訊息因歷史載入 + WS 事件而重覆插入）
const APPENDED_IDS = new Set()
...
function appendMessageRow(messageId, senderId, senderName, content, createdAt) {
    // 去重：若已有相同 messageId 的訊息，不再插入
    if (messageId != null) {
        const key = String(messageId)
        if (APPENDED_IDS.has(key)) return
    }
    ...
    if (messageId != null) {
        APPENDED_IDS.add(String(messageId))
    }
}
