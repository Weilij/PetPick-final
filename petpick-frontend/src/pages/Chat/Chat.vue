...
const pendingRows = new Map() // clientId -> DOM row（避免重複）

// ---- Fallback polling (when WS misses events) ----
let pollTimer = null
const seenIds = new Set()    // messageId set to avoid duplicates
let lastCreatedAt = null     // ISO string of the latest message we have

// 工具
function esc(s) { return String(s || '').replace(/[&<>"']/g, m => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[m])) }
function pad(n) { return String(n).padStart(2, '0') }
function fmtTime(iso) { const d = new Date(iso); return `${pad(d.getHours())}:${pad(d.getMinutes())}` }
function isMessage(ev) { return !!ev && (ev.messageId != null || (ev.content && String(ev.content).trim().length > 0)) }
function $rowByClientId(id){ return document.querySelector(`.msg-row[data-client-id="${id}"]`) }
function $rowByMsgId(id){ return document.querySelector(`.msg-row[data-id="${id}"]`) }

// 等 userId
async function waitForUserId(maxMs = 3000) {
  const start = Date.now()
  while (!UID.value && (Date.now() - start) < maxMs) {
    await new Promise(r => setTimeout(r, 60))
  }
  if (!UID.value) throw new Error('userId 尚未就緒')
}

onMounted(async () => {
  try { await waitForUserId() } catch { /* 仍嘗試載入，但很多功能會失效 */ }
  ...
  // —— WS：等 REST 正常後再開 —— //
  try { if (UID.value) Realtime.init(UID.value) } catch { }

  // 確保在目前會話下啟動 fallback 輪詢
  startPolling()
})
...
async function loadConversations() {
  const uid = UID.value
  const { data } = await axios.get(API.listConvs(uid))
  ...
  CONVS.forEach(c => {
    ...
    try {
      Realtime.subscribeConversationLite?.(c.conversationId, (ev) => {
        ...
        if (String(ev.senderId) === String(UID.value)) return
...
async function openConversation(cid) {
  ...
  box.innerHTML = ''
  const { data: msgs } = await axios.get(API.listMsgs(cid, UID.value))
  if (Array.isArray(msgs)) {
    for (const m of msgs) {
      // 避免重複：紀錄已呈現的 messageId
      if (m.messageId != null) seenIds.add(String(m.messageId))
      if (!lastCreatedAt || (m.createdAt && m.createdAt > lastCreatedAt)) {
        lastCreatedAt = m.createdAt
      }
      appendMessageRow(m.messageId, m.senderId, m.senderName, m.content, m.createdAt)
    }
  }
  box.scrollTop = box.scrollHeight;

  // 標記已讀（REST）
  try { await axios.post(API.markRead(cid, UID.value)) } catch { }
  ...
  try {
    ...
    unsubCurr = Realtime.subscribeConversation(cid, async (ev) => {
      if (!ev) return
      // typing
      if (ev.type === 'typing' && String(ev.senderId) !== String(UID.value)) {
      ...
      // read
      if (ev.type === 'read' && String(ev.userId) !== String(UID.value)) {
      ...
      // message
      if (isMessage(ev)) {
        // 如果是我剛剛送出的、且帶有 clientId，就不要再插一顆
        if (ev.clientId) {
          const mine = $rowByClientId(ev.clientId)
          if (mine) {
            mine.dataset.id = ev.messageId ?? ''
            const meta = mine.querySelector('.meta')
            if (meta && ev.createdAt) meta.textContent = `你　${fmtTime(ev.createdAt)}`
            pendingRows.delete(ev.clientId)
            // 更新追蹤
            if (ev.messageId != null) seenIds.add(String(ev.messageId))
            if (!lastCreatedAt || (ev.createdAt && ev.createdAt > lastCreatedAt)) lastCreatedAt = ev.createdAt
            return
          }
        }
        // 不是我剛剛送出的：避免重複，再插入
        if (ev.messageId != null && seenIds.has(String(ev.messageId))) {
          // 已有，不再插入
        } else {
          appendMessageRow(ev.messageId, ev.senderId, ev.senderName, ev.content, ev.createdAt)
          if (ev.messageId != null) seenIds.add(String(ev.messageId))
          if (!lastCreatedAt || (ev.createdAt && ev.createdAt > lastCreatedAt)) lastCreatedAt = ev.createdAt
          const box2 = $chatMsgsEl(); if (box2) box2.scrollTop = box2.scrollHeight
        }
        if (document.visibilityState === 'visible' && String(ev.senderId) !== String(UID.value)) {
          try { await axios.post(API.markRead(cid, UID.value)) } catch { }
          try { Realtime.sendRead(cid, UID.value) } catch { }
        }
      }
    })
  } catch { }
...
function bindSend() {
  ...
  input.oninput = () => {
    try {
      if (currentConvId) Realtime.sendTyping(currentConvId, UID.value)
    } catch { }
  }
}
...
function doSend() {
  ...
  const row = appendMessageRow(null, UID.value, '你', text, nowIso, clientId) // 帶 clientId
  const boxAfterInsert = $chatMsgsEl();
  if (boxAfterInsert) boxAfterInsert.scrollTop = boxAfterInsert.scrollHeight;
  pendingRows.set(clientId, row)
  ...
  try {
    ...
    if (meta) meta.textContent = `${meta.textContent}（傳送失敗）`
  }
}
...
function appendMessageRow(messageId, senderId, senderName, content, createdAt, clientId) {
  if (!String(content || '').trim()) return
  const isMe = String(senderId) === String(UID.value)
...
function startPolling() {
  stopPolling()
  // 只有在有目前會話、而且 UID 存在時才輪詢
  if (!currentConvId || !UID.value) return
  pollTimer = setInterval(async () => {
    try {
      const { data: msgs } = await axios.get(API.listMsgs(currentConvId, UID.value))
      if (!Array.isArray(msgs)) return
      for (const m of msgs) {
        const idStr = m.messageId != null ? String(m.messageId) : null
        // 跳過已經顯示過的
        if (idStr && seenIds.has(idStr)) continue
        // 不是我剛剛送的 duplicate（沒有 messageId 但 content 一樣的）一律允許，交由 UI 去重
        appendMessageRow(m.messageId, m.senderId, m.senderName, m.content, m.createdAt)
        if (idStr) seenIds.add(idStr)
        if (!lastCreatedAt || (m.createdAt && m.createdAt > lastCreatedAt)) lastCreatedAt = m.createdAt
      }
    } catch (err) {
      // 輪詢失敗忽略（可能暫時未登入或網路中斷）
    }
